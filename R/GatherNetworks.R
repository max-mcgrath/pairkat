...
GatherNetworks <- function(SE,
  ...
  valid_species <- as.data.frame(keggList("organism"))$organism
  ...
  if (!(keggID %in% colnames(pD))){
    stop(paste(keggID,"not found in SummarizedExperiment object rowData"))
  }
  
  # remove metabolites with zero variance
  remove_vars <- colnames(tmD)[!as.vector(
    unlist(lapply(tmD, function(x) length(unique(x)) > 1)))]
  
  # throw warning if metabolites were removed
  if (length(remove_vars) > 0){
    warning("Removed the following metabolites from analysis due to 
            insufficient variance:",paste(remove_vars, collapse = ", "))
    tmD <- tmD[,!(colnames(tmD) %in% remove_vars)]
  }
  
  # compile pathway data
  pdat <- pathList(pathDat = pD, 
                   .pathID = keggID,
                   ms = minPathwaySize, 
                   s_id = species)
  
  # gather networks
  nn <- getNetworks(pathDat = pD,
                    metab = tmD,
                    database = "KEGG",
                    pdat = pdat,
                    pathID = keggID)
...
pathList <- function(pathDat, .pathID, ms, s_id){
  ...
  comps <- lapply(hsapath, function(p) keggGet(p)[[1]])
  names(comps) <- hsapath
  comp <- lapply(comps, function(p) names(p$COMPOUND))
