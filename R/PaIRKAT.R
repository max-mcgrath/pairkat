...
#' gave reasonable results, as over-smoothing can lead to inflated TypeI errors. 
#'
...
PaIRKAT <- function(formula.H0, networks, tau = 1){
  ...
  if (length(formula.H0[[3]]) > 1){
    for (i in 1:length(formula.H0[[3]])){
  ...
  if(length(unique(outcome_vector)) == 2){
    out.type <- "binary"
    message("Binary outcome detected. Null model will be a glm with logit link")
  } else if(length(unique(outcome_vector)) > 2){
    out.type <- "continuous"
    message("Continuous outcome detected. Null model will be a linear model.")
  }
  ...
  if(!is.numeric(outcome_vector)){
    if (out.type == "binary"){
      ...
      warning(paste("Binary outcome is non-numeric, encoding",
                    pos_level,"as 1 and",ref_level,"as 0."))
    }
    else{
  ...
  for (i in 1:length(networks$networks)){
...
Gaussian_kernel <- function(rho, Z){
  exp(-(1/rho)*as.matrix(dist(Z, method = "euclidean", upper = TRUE)^2))
}
...
SKAT.c <- function(formula.H0, .data = NULL, K,
  ...
  p.value <- KAT.pval(0, lambda=sort(lambda, decreasing = TRUE), 
                      acc = acc, 
                      lim = lim)
...
saddle = function(x,lambda){
  ...
  kprime0 = function(zeta) sapply(zeta, 
